...
export const UploadMultipleImages = ({
  ...
  initialImages,
  limit,
}: IUploadMultipleImages) => {
  const [imgsList, setImgsList] = useState<(IImgData | string)[]>(
    initialImages || []
  );
  ...
  const handleAddImage = (img: File) => {
    ...
    setImgsList((i) => [...i, { file: img, url: imgUrl }]);
  };

  const handleDeleteImage = (index: number) => {
    const selectedImage = imgsList[index];
    if (typeof selectedImage !== "string")
      window.URL.revokeObjectURL(selectedImage.url);
    setImgsList((i) => i.filter((_, j) => j !== index));
  };
