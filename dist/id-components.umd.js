(function(g,d){typeof exports=="object"&&typeof module<"u"?d(exports,require("styled-components"),require("react/jsx-runtime"),require("react"),require("react-loading-skeleton"),require("react-select"),require("@radix-ui/react-collapsible"),require("@floating-ui/react-dom")):typeof define=="function"&&define.amd?define(["exports","styled-components","react/jsx-runtime","react","react-loading-skeleton","react-select","@radix-ui/react-collapsible","@floating-ui/react-dom"],d):(g=typeof globalThis<"u"?globalThis:g||self,d(g["ID-Components"]={},g.styled,g["react/jsx-runtime"].js,g.React,g.Skeleton,g.Select,g["@radix-ui/react-collapsible"],g["@floating-ui/react-dom"]))})(this,function(g,d,t,x,ae,de,le,D){"use strict";const W=e=>e&&typeof e=="object"&&"default"in e?e:{default:e};function ce(e){if(e&&e.__esModule)return e;const r=Object.create(null,{[Symbol.toStringTag]:{value:"Module"}});if(e){for(const o in e)if(o!=="default"){const a=Object.getOwnPropertyDescriptor(e,o);Object.defineProperty(r,o,a.get?a:{enumerable:!0,get:()=>e[o]})}}return r.default=e,Object.freeze(r)}const f=W(d),se=W(ae),fe=W(de),E=ce(le),he={bold:700,medium:500,regular:400,light:300},O={xs:"0.750rem",sm:"0.875rem	",md:"1.0rem",lg:"1.250rem",xl:"1.5rem","2xl":"1.875rem","3xl":"2.0rem","4xl":"2.5rem","5xl":"3.0rem"},V={h1:"xl",h2:"lg",h3:"md",h4:"sm",h5:"xs",h6:"xs",text:"sm"},c={color:{NEUTRAL:{0:"#FFFFFF",20:"#FFFFFF",50:"#F5F5F5",100:"#EBEBEB",200:"#dfdfdf",300:"#bfbfbf",400:"#9f9f9f",500:"#808080",600:"#404040",700:"#383838",800:"#202020",900:"#000000"},PRIMARY:{0:"#FFFFFF",50:"#F1F2F9",100:"#E6E8F4",200:"#DDDFF0",300:"#BBBFE2",400:"#999FD3",500:"#777fc5",600:"#545fb6",700:"#414a98",800:"#333a76",900:"#242954"},SECONDARY:{0:"#FFFFFF",20:"#F7F5FF",50:"#EFEBFF",100:"#E6E0FF",200:"#d0c5ff",300:"#d0c5ff",400:"#b9a8ff",500:"#a18cff",600:"#8a6fff",700:"#7252ff",800:"#5b35ff",900:"#4318FF"},TERTIARY:{0:"#FFFFFF",50:"#FFFAFC",100:"#FFF5F9",200:"#ffeff5",300:"#ffe0eb",400:"#ffd0e1",500:"#ffc1d7",600:"#ffb1cd",700:"#FFA1C3",800:"#ff92b9",900:"#FF82AF"},QUATERNARY:{0:"#FFFFFF",50:"#FBFCFE",100:"#F2F6FD",200:"#ebf1fc",300:"#D8E4FA",400:"#c4d6f8",500:"#b0c8f5",600:"#9cbbf3",700:"#88adf0",800:"#75a0ed",900:"#6192EB"},STATE:{0:"#FFFFFF",50:"#FFFBF5",100:"#FEF7EB",200:"#fef3e3",300:"#fde7c6",400:"#fcdba9",500:"#fbcf8d",600:"#fac270",700:"#f9b654",800:"#f8aa38",900:"#F79E1B"},ALERT:{0:"#FFFFFF",50:"#FFFBF5",100:"#FEEDEC",200:"#fde3e2",300:"#fbc8c5",400:"#f9ada8",500:"#f7918b",600:"#f4756d",700:"#F25A50",800:"#F03E33",900:"#EE2316"},SUCCESS:{0:"#FFFFFF",50:"#F7FDFA",100:"#EEFBF4",200:"#e4f9ed",300:"#c9f3db",400:"#aeedc9",500:"#93e7b7",600:"#78e1a4",700:"#5DDB92",800:"#42D580",900:"#2DC96F"},OTHER:{color1:"#151E5A"}},effect:{darkShadow:{4:"0px 0px 4px 1px rgba(139, 139, 139, 0.32)",6:"0px 1px 6px rgba(60, 60, 60, 0.2)",8:"0px 1px 6px rgba(107, 107, 107, 0.2)",12:"0px 0px 12px 2px #F6F6F6",24:"0px 0px 24px 4px rgba(0, 14, 51, 0.2)",30:"0px 10px 30px rgba(0, 0, 0, 0.08)"},lightShadow:{2:"0px 0px 2px 1px rgba(149, 178, 254, 0.2)",16:"0px 4px 16px rgba(153, 181, 255, 0.2)"}}},ue=({shape:e})=>{switch(e){case"rounded":return d.css`
        border-radius: 0.625rem;
      `;case"squared":return d.css`
        border-radius: 0;
      `;case"circular":return d.css`
        border-radius: 10rem;
      `;default:return d.css`
        border-radius: 0.625rem;
      `}},X=({design:e,color:r})=>{switch(e){case"solid":return d.css`
        background-color: ${()=>C({color:r})};
        color: #fff;
      `;case"outline":return d.css`
        background-color: white;
        border: 1px solid ${()=>C({color:r})};
        color: ${()=>C({color:r})};
      `;case"flat":return d.css`
        background-color: white;
        border: none;
        color: ${()=>C({color:r})};
      `;case"link":return d.css`
        background-color: transparent;
        border: none;
        color: ${()=>C({color:r})};

        &:hover {
          color: ${()=>C({color:r,hover:!0})};
          text-decoration: underline;
        }
      `;default:return d.css`
        background-color: ${()=>C({color:r})};
        color: #fff;
      `}},C=({color:e="SECONDARY/900",design:r,hover:o})=>{const a=e.split("/"),i=a[0];let n=+a[1];if(o&&n>=300)switch(r){case"solid":n-=100;break;case"outline":n=100;break;case"flat":n=100;break;default:n>=400?n-=100:n+=100;break}if(o&&n<300)switch(r){case"solid":n+=100;break;case"outline":n=100;break;case"flat":n=100;break;default:n-=100;break}return i==="PRIMARY"||i==="NEUTRAL"||i==="SECONDARY"||i==="TERTIARY"||i==="QUATERNARY"||i==="STATE"||i==="ALERT"||i==="SUCCESS"?n===50||n===100||n===200||n===300||n===400||n===500||n===600||n===700||n===800||n===900?c.color[i][n]:c.color[i][900]:c.color.PRIMARY[900]},Z=({fullWidth:e})=>e?d.css`
      width: 100%;
    `:d.css`
    width: auto;
  `,pe=({isLoading:e})=>e?d.css`
      cursor: not-allowed;
      opacity: 0.5;
    `:d.css`
    cursor: pointer;
    opacity: 1;
  `,ge=({disabled:e})=>e?d.css`
      cursor: not-allowed;
      opacity: 0.5;
    `:d.css`
    cursor: pointer;
    opacity: 1;
  `,P=({size:e="sm"})=>d.css`
    font-size: ${O[e]||e};
  `,xe=({align:e})=>e?d.css`
      text-align: ${e};
    `:d.css`
    text-align: left;
  `,J=e=>{let r="";return e.m&&(r+=`
      margin-top: ${e.m};
      margin-bottom: ${e.m};
      margin-left: ${e.m};
      margin-right: ${e.m};
    `),e.mb&&(r+=`
      margin-bottom: ${e.mb};
    `),e.mt&&(r+=`
      margin-top: ${e.mt};
    `),e.ml&&(r+=`
      margin-left: ${e.ml};
    `),e.mr&&(r+=`
      margin-right: ${e.mr};
    `),e.mx&&(r+=`
      margin-left: ${e.mx};
      margin-right: ${e.mx};
    `),e.my&&(r+=`
      margin-top: ${e.my};
      margin-bottom: ${e.my};
    `),e.p&&(r+=`
      padding-top: ${e.p};
      padding-bottom: ${e.p};
      padding-left: ${e.p};
      padding-right: ${e.p};
    `),e.pb&&(r+=`
      padding-bottom: ${e.pb};
    `),e.pt&&(r+=`
      padding-top: ${e.pt};
    `),e.pl&&(r+=`
      padding-left: ${e.pl};
    `),e.pr&&(r+=`
      padding-right: ${e.pr};
    `),e.px&&(r+=`
      padding-left: ${e.px};
      padding-right: ${e.px};
    `),e.py&&(r+=`
      padding-top: ${e.py};
      padding-bottom: ${e.py};
    `),d.css`
    ${r}
  `},be=f.default.button`
  background-color: transparent;
  cursor: pointer;
  border: none;
`,me=f.default(be)`
  display: inline-flex;
  gap: 0.5rem;
  font-weight: 500;
  align-items: center;
  justify-content: center;
  transition: background 0.3s ease-out;
  padding: ${({design:e})=>e==="link"?"0":"0.7rem"};
  min-height: ${({design:e})=>e==="link"?"0":"45px"};

  ${({size:e})=>P({size:e})};
  ${({shape:e})=>ue({shape:e})};
  ${({design:e,color:r})=>X({design:e,color:r})};
  ${({fullWidth:e})=>Z({fullWidth:e})};
  ${({isLoading:e})=>pe({isLoading:e})};
  ${({disabled:e})=>ge({disabled:e})};
  ${({m:e,mb:r,mt:o,mr:a,ml:i,my:n,mx:l,p:u,pb:p,pt:h,pr:s,pl:b,py:k,px:v})=>J({m:e,mb:r,mt:o,mr:a,ml:i,my:n,mx:l,p:u,pb:p,pt:h,pr:s,pl:b,py:k,px:v})};

  &:hover {
    background-color: ${({color:e,design:r})=>r==="link"?"transparent":C({color:e,design:r,hover:!0})};
  }

  &:active {
    background-color: ${({color:e,design:r})=>r==="link"?"transparent":C({color:e,design:r,hover:!0})};
  }

  &:focus {
    outline: none;
  }

  &:disabled {
    cursor: not-allowed;
  }

  &:disabled:hover {
    ${({design:e,color:r})=>X({design:e,color:r})};
  }
`,z=({children:e,...r})=>t.jsxs(me,{...r,children:[r.leftIcon,e,r.rightIcon]});function ve(e,r,o){const a=()=>{const i=document.querySelectorAll("input");return Array.from(i).filter(l=>l.className===r)};x.useEffect(()=>{const i=n=>{n.key==="Backspace"&&setTimeout(()=>{const l=a(),u=document.activeElement;u.value="";const p=l.indexOf(u);p>0&&l[p-1].focus()},0)};return document.addEventListener("keydown",i),()=>document.removeEventListener("keydown",i)},[]),x.useEffect(()=>{const i=n=>{if(n.key!=="Backspace"){const l=n.key.toLowerCase();setTimeout(()=>{if(l.length===1){const u=a(),p=document.activeElement;p.value=l;const h=u.indexOf(p);h<u.length-1&&u[h+1].focus()}},0)}};return document.addEventListener("keydown",i),()=>document.removeEventListener("keydown",i)},[]),x.useEffect(()=>{if(e){const i=n=>{const l=n.key.toLowerCase();(l.length===1||l==="backspace")&&setTimeout(()=>{const p=a().map(h=>h.value).join("");e(p)},0)};return document.addEventListener("keydown",i),()=>document.removeEventListener("keydown",i)}},[]),x.useEffect(()=>{if(o){const i=a(),n=i.map(p=>p.value),l=o.split(""),u=l.length===n.length?l:n;i.forEach((p,h)=>{const s=u[h];s&&(p.value=s)})}},[o])}const we=d.keyframes`
  0% {
    opacity: 0;
    transform: translateY(-4px);
  }

  80% {
    opacity: 0.5;
  }

  100% {
    opacity: 1;
    transform: translateY(0);
  }
`,U=f.default.div`
  opacity: 0;
  ${({shouldAppear:e})=>e&&d.css`
      animation: ${we} 0.2s ease-in;
      animation-fill-mode: forwards;
    `}
`,ke={h1:"h1",h2:"h2",h3:"h3",h4:"h4",h5:"h5",h6:"h6",p:"p",li:"li",span:"span",strong:"strong",label:"label"},Ce=d.css`
  display: inline-block;
  font-family: "Poppins";
  font-style: normal;
  line-height: 24px;
  letter-spacing: -0.04rem;
  ${({align:e})=>xe({align:e})}
  ${({m:e,mb:r,mt:o,mr:a,ml:i,my:n,mx:l,p:u,pb:p,pt:h,pr:s,pl:b,py:k,px:v})=>J({m:e,mb:r,mt:o,mr:a,ml:i,my:n,mx:l,p:u,pb:p,pt:h,pr:s,pl:b,py:k,px:v})};
`,Ae=e=>{const r=e==="h1"||e==="h2"||e==="h3"||e==="h4"||e==="h5"||e==="h6",o=r?"bold":"medium",a=r?V[e]:V.text,i="PRIMARY/900";return d.css`
    font-weight: ${({weight:n=o})=>he[n]||"500"};
    font-size: ${({size:n=a})=>O[n]||n};
    color: ${({color:n=i})=>{const l=n==null?void 0:n.split("/")[0],u=Number(n==null?void 0:n.split("/")[1]);return c.color[l][u]}};
  `},Ne=f.default.div`
  ${Ce}
  ${({as:e})=>Ae(e)}
`,N=({as:e,...r})=>{const o=ke[e];return t.jsx(Ne,{as:o,...r})},Fe={H1:e=>t.jsx(N,{as:"h1",...e}),H2:e=>t.jsx(N,{as:"h2",...e}),H3:e=>t.jsx(N,{as:"h3",...e}),H4:e=>t.jsx(N,{as:"h4",...e}),H5:e=>t.jsx(N,{as:"h5",...e}),H6:e=>t.jsx(N,{as:"h6",...e})},ye=f.default.div`
  display: flex;
  align-items: center;
  gap: 8px;
  transition: 0.3s ease-out;
`,w=e=>e.rightIcon||e.leftIcon?t.jsxs(ye,{children:[e.leftIcon,t.jsx(N,{as:e.as||"p",...e}),e.rightIcon]}):t.jsx(N,{as:e.as||"p",...e}),ze=f.default.div`
  display: grid;
  grid-template-columns: repeat(${e=>e.length}, 1fr);
  grid-gap: 22px;
  align-items: center;

  > input {
    text-align: center;
    width: 100%;
    flex-grow: 1;
    border: none;
    outline: none;
    width: 45px;
    height: 45px;
    border: 1px solid
      ${({errorMessage:e,success:r})=>e?c.color.ALERT[900]:r?c.color.SUCCESS[900]:c.color.NEUTRAL[300]};
    border-radius: 8px;
    background: #fff;
    font: normal 16px "Poppins";
    letter-spacing: -0.04em;
    color: ${()=>c.color.NEUTRAL[700]};

    transition: background 0.3s ease-out;
    transition: border 0.3s ease-out;

    &:disabled {
      cursor: not-allowed;
      background-color: ${()=>c.color.NEUTRAL[100]};
      color: ${()=>c.color.NEUTRAL[500]};
    }

    &:focus {
      border-color: ${()=>c.color.QUATERNARY[900]};
    }
  }
`,Te=f.default.div`
  display: flex;
  align-items: center;
  flex-direction: column;
  gap: 10px;
`,Le=({id:e,value:r,length:o=4,errorMessage:a,success:i,onChange:n=()=>{},...l})=>(ve(n,e,r),t.jsxs(Te,{children:[t.jsx(ze,{length:o,success:i,errorMessage:a,children:Array.from({length:o}).map((u,p)=>t.jsx("input",{id:p.toString(),className:e,maxLength:1,...l},p))}),a&&t.jsx(U,{shouldAppear:Boolean(a),children:t.jsx(w,{size:"xs",weight:"light",color:"ALERT/900",children:a})})]})),Se=f.default.div`
  position: relative;
  width: 100%;

  img {
    margin-right: 0.25rem;
  }

  label {
    display: inline-block;
    margin-bottom: 0.25rem;
    padding-left: 0.05rem;
  }

  select + svg {
    position: absolute;
    right: 20px;
    fill: #151e5a;
    top: 50%;
    transform: translateY(-50%);
  }

  label + select + svg {
    position: absolute;
    right: 20px;
    fill: #151e5a;
    top: 45px;
    transform: none;
  }

  > small {
    display: block;
    font: normal 11px Poppins;
    color: ${()=>c.color.ALERT[900]};
    margin-left: 2px;
    margin-top: 3px;
  }

  & > span {
    width: 100%;
  }
`,Ie={control:(e,r)=>({...e,minHeight:"45px",borderRadius:10,backgroundColor:"#ffffff",border:`1px solid ${r.isFocused?c.color.QUATERNARY[900]:c.color.NEUTRAL[300]}`,boxShadow:"none",cursor:"pointer"}),singleValue:()=>({color:c.color.PRIMARY[900],fontWeight:500,fontSize:"14px"}),placeholder:e=>({...e,color:c.color.NEUTRAL[700]}),indicatorSeparator:e=>({...e,display:"none",padding:0}),indicatorsContainer:e=>({...e,"> div":{padding:"0",paddingRight:"3px"}}),valueContainer:e=>({...e,padding:0,paddingLeft:"6px"})},We=({formatOptionLabel:e,options:r,loading:o=!1,style:a,error:i,firstOption:n,initialValue:l,value:u,onChange:p,isMulti:h,placeholder:s,label:b})=>{const k=n||{label:"Seleccione una opci\xF3n",value:""},v=[...r];return t.jsxs(Se,{style:a,hasError:!!i,children:[b&&t.jsx(w,{as:"label",align:"left",size:"sm",color:"NEUTRAL/700",children:b}),o?t.jsx(se.default,{style:{borderRadius:10},className:"loading-skeleton",height:46,width:"100%"}):t.jsx(fe.default,{placeholder:s,value:u||void 0,defaultValue:l||k,options:v,onChange:p,styles:Ie,isMulti:h,formatOptionLabel:e})]})},Ee=f.default.div`
  width: ${e=>e.size}px;
  height: ${e=>e.size}px;
  position: relative;
  &::before {
    content: "";
    position: absolute;
    top: 50%;
    left: 50%;
    transform: translate(-50%, -50%);
    background: transparent;
    width: ${e=>e.size}px;
    height: ${e=>e.size}px;
    border-radius: ${e=>e.borderRadius||"50%"};
    transition: 0.3s background, 0.3s width, 0.3s height;
  }
  &:hover ::before {
    width: ${e=>e.size*2}px;
    height: ${e=>e.size*2}px;
    background: ${()=>c.color.NEUTRAL[50]};
  }
  &::after {
    content: "";
    position: absolute;
    top: 50%;
    left: 50%;
    transform: translate(-50%, -50%);
    background: transparent;
    width: ${e=>e.size*2}px;
    height: ${e=>e.size*2}px;
    border-radius: ${e=>e.borderRadius||"50%"};
    transition: 0.3s background, 0.3s width, 0.3s height;
    opacity: 0.3;
  }
  &:active ::after {
    width: ${e=>e.size*2}px;
    height: ${e=>e.size*2}px;
    background: #fff;
  }

  ${e=>e.variant=="V1"&&d.css`
      ${r=>r.active&&d.css`
          &::before {
            content: "";
            position: absolute;
            top: 50%;
            left: 50%;
            transform: translate(-50%, -50%);
            background: ${o=>o.color||c.color.PRIMARY[900]};
            width: ${o=>o.size*2}px;
            height: ${o=>o.size*2}px;
            border-radius: ${o=>o.borderRadius||"50%"};
            transition: 0.3s background;
          }
          &:hover ::before {
            width: ${o=>o.size*2}px;
            height: ${o=>o.size*2}px;
            background: ${o=>o.color||c.color.PRIMARY[900]};
          }
        `}
    `}
  ${e=>e.variant=="V2"&&d.css`
      ${r=>r.active&&d.css`
          &::before {
            content: "";
            position: absolute;
            top: 50%;
            left: 50%;
            transform: translate(-50%, -50%);
            background: transparent;
            width: ${o=>o.size}px;
            height: ${o=>o.size}px;
            border-radius: ${o=>o.borderRadius||"50%"};
            transition: 0.3s background;
          }
          &:hover ::before {
            width: ${o=>o.size*2}px;
            height: ${o=>o.size*2}px;
            background: ${()=>c.color.NEUTRAL[50]};
          }
        `}
    `}
`,Pe=f.default.svg`
  position: absolute;
  top: 50%;
  left: 50%;
  transform: translate(-50%, -50%);
  width: ${e=>e.size}px;
  height: ${e=>e.size}px;
  fill: ${e=>e.color||c.color.PRIMARY[900]};
  ${e=>e.variant==="V1"&&d.css`
      ${r=>r.active&&d.css`
          fill: #fff;
        `}
    `}
  ${e=>e.variant==="V2"&&d.css`
      fill: ${()=>c.color.NEUTRAL[400]};
      ${r=>r.active&&d.css`
          fill: ${o=>o.color||c.color.PRIMARY[900]};
        `}
    `}
`,q=e=>{const{children:r,color:o,size:a,viewBox:i,active:n,borderRadius:l,variant:u}=e;return t.jsx(Ee,{active:n,size:a||24,borderRadius:l||"50%",variant:u||"V1",color:o,children:t.jsx(Pe,{active:n,color:o,size:a||24,viewBox:i||"0 0 32 32",variant:u||"V1",children:r})})},Ue=e=>{const{color:r,size:o}=e;return t.jsx(q,{color:r,size:o,viewBox:"0 0 22 13",fill:"none",children:t.jsx("path",{d:"M19 1.406s-.32.772-1 1.752m-7 4.031a7.587 7.587 0 0 1-4.051-1.157M11 7.19a7.587 7.587 0 0 0 4.051-1.157M11 7.19v4.048m-8-9.83s.354.851 1.106 1.901M6.95 6.033 4 9.502m2.949-3.47c-1.26-.778-2.198-1.824-2.843-2.724m10.945 2.724 2.449 3.47m-2.449-3.47C16.381 5.21 17.354 4.09 18 3.158m-13.894.15L1 4.877m17-1.718 3 1.718",stroke:"#242954",strokeWidth:2,strokeLinecap:"round"})})},je=e=>{const{color:r,size:o}=e;return t.jsx(q,{color:r,size:o,viewBox:"0 0 22 19",fill:"none",children:t.jsx("path",{fillRule:"evenodd",clipRule:"evenodd",d:"M11 2.75c-3.089 0-6.002 1.167-8.524 3.239a.75.75 0 1 1-.952-1.16C4.274 2.57 7.515 1.25 11 1.25c3.485 0 6.726 1.32 9.476 3.58a.75.75 0 1 1-.952 1.159C17.002 3.917 14.089 2.75 11 2.75Zm-8.249 8.454v.002c.006.02.025.103.1.259.09.187.234.424.434.697.4.546.99 1.19 1.73 1.804 1.488 1.238 3.487 2.284 5.639 2.284 2.163 0 4.343-1.055 6.016-2.308.829-.62 1.506-1.27 1.968-1.819.231-.276.398-.514.503-.7a1.31 1.31 0 0 0 .102-.219 1.315 1.315 0 0 0-.102-.218 4.803 4.803 0 0 0-.503-.7c-.462-.55-1.14-1.198-1.968-1.82a13.63 13.63 0 0 0-1.87-1.179c.264.57.412 1.203.412 1.872 0 2.496-2.052 4.5-4.558 4.5s-4.558-2.004-4.558-4.5c0-.615.125-1.2.35-1.734-.515.309-.995.655-1.432 1.018a10.49 10.49 0 0 0-1.729 1.804c-.2.273-.343.51-.434.697-.075.156-.094.238-.1.26 0 0 0 0 0 0Zm7.903-6.545c-2.627 0-4.955 1.264-6.599 2.63a11.982 11.982 0 0 0-1.98 2.072 6.141 6.141 0 0 0-.575.931c-.13.27-.25.594-.25.912 0 .319.12.643.25.913.14.29.337.606.575.931a11.98 11.98 0 0 0 1.98 2.071c1.644 1.367 3.972 2.631 6.599 2.631 2.616 0 5.11-1.255 6.915-2.607.91-.682 1.673-1.408 2.217-2.055.271-.323.498-.638.662-.93.15-.265.302-.606.302-.954 0-.347-.153-.688-.302-.953a6.264 6.264 0 0 0-.662-.93c-.544-.647-1.307-1.373-2.217-2.055-1.806-1.352-4.299-2.607-6.915-2.607Zm0 1.5c-1.7 0-3.058 1.354-3.058 3s1.358 3 3.058 3c1.7 0 3.058-1.354 3.058-3s-1.359-3-3.058-3Z",fill:"#242954",stroke:"#242954",strokeLinecap:"round"})})},Me=e=>t.jsx("svg",{stroke:"currentColor",fill:"currentColor",strokeWidth:0,color:e.color||"#EE2316",height:24,width:24,viewBox:"0 0 24 24",...e,children:t.jsxs("g",{stroke:"none",children:[t.jsx("path",{fill:"none",d:"M0 0h24v24H0z"}),t.jsx("path",{d:"M12 22C6.477 22 2 17.523 2 12S6.477 2 12 2s10 4.477 10 10-4.477 10-10 10zm0-2a8 8 0 1 0 0-16 8 8 0 0 0 0 16zm-1-5h2v2h-2v-2zm0-8h2v6h-2V7z"})]})}),He=e=>t.jsx("svg",{stroke:"currentColor",fill:"currentColor",strokeWidth:0,color:e.color||"#2DC96F",height:20,width:20,viewBox:"0 0 1024 1024",...e,children:t.jsx("path",{d:"M912 190h-69.9c-9.8 0-19.1 4.5-25.1 12.2L404.7 724.5 207 474a32 32 0 0 0-25.1-12.2H112c-6.7 0-10.4 7.7-6.3 12.9l273.9 347c12.8 16.2 37.4 16.2 50.3 0l488.4-618.9c4.1-5.1.4-12.8-6.3-12.8z",stroke:"none"})}),Ye=f.default.div`
  ${({fullWidth:e})=>Z({fullWidth:e})};
  position: relative;
  display: flex;
  align-items: center;
  gap: 0.5rem;
  border-radius: 8px;
  border: 1px solid
    ${({error:e,success:r,isFocused:o})=>e?c.color.ALERT[900]:r&&o?c.color.SUCCESS[900]:o?c.color.QUATERNARY[900]:c.color.NEUTRAL[300]};
  background: #fff;
  min-height: 45px;
  ::placeholder {
    color: ${()=>c.color.NEUTRAL[500]};
  }

  background-color: ${({disabled:e})=>e?c.color.NEUTRAL[100]:"#fff"};
  cursor: ${({disabled:e})=>e?"not-allowed":"auto"};
  transition: background 0.3s ease-out;
  transition: border 0.3s ease-out;

  > input {
    border-radius: 8px;
    width: 100%;
    ${({size:e})=>P({size:e})};
    flex-grow: 1;
    border: none;
    outline: none;
    padding: 10px 0px;
    background-color: #fff;
    letter-spacing: -0.04em;
    color: ${()=>c.color.NEUTRAL[700]};
    padding-left: ${({leftAddon:e})=>e?"0px":"12px"};
    padding-right: ${({rightAddon:e})=>e?"0px":"12px"};
    padding-right: ${({success:e,error:r})=>(e||r)&&"2.5rem"};

    &:disabled {
      cursor: not-allowed;
      background-color: ${()=>c.color.NEUTRAL[100]};
      color: ${()=>c.color.NEUTRAL[500]};
    }
  }

  .leftAddon {
    padding-left: 12px;
  }

  .rightAddon {
    padding-right: 12px;
  }

  span {
    color: ${()=>c.color.NEUTRAL[500]};
    ${({size:e})=>P({size:e})};
    display: flex;
    align-items: center;
    justify-content: center;
    letter-spacing: -0.04em;
    user-select: none;
  }

  .state {
    position: absolute;
    right: 1rem;
  }
`,j=x.forwardRef(({type:e,leftAddon:r,rightAddon:o,placeholder:a,error:i,success:n,size:l,fullWidth:u,name:p,infoMessage:h,onFocus:s,onBlur:b,onChange:k,...v},T)=>{const[F,y]=x.useState(!1),L=!i&&!n&&h&&F;return t.jsxs(t.Fragment,{children:[t.jsxs(Ye,{fullWidth:u,success:n,size:l,error:i,disabled:v.disabled,leftAddon:r,rightAddon:o,isFocused:F,children:[r&&t.jsx("span",{className:"leftAddon",children:r}),t.jsx("input",{ref:T,onFocus:S=>{s&&s(S),y(!0)},onBlur:S=>{b&&b(S),y(!1)},onChange:k,placeholder:a,type:e,name:p,...v}),o&&t.jsx("span",{className:"rightAddon",children:o}),i&&t.jsx("span",{className:"state",children:t.jsx(Me,{size:24,color:c.color.ALERT[900]})}),n&&t.jsx("span",{className:"state",children:t.jsx(He,{size:20,color:c.color.SUCCESS[900]})})]}),L&&t.jsx(U,{shouldAppear:L,children:t.jsx(w,{size:"xs",color:"QUATERNARY/900",children:h})})]})});j.displayName="TextInput";const De=f.default.div`
  width: ${e=>e.fullWidth?"100%":"auto"};

  .label-container {
    display: flex;
    justify-content: space-between;
  }

  label {
    display: inline-block;
    margin-bottom: 0.25rem;
    padding-left: 0.05rem;
  }
`,G=x.forwardRef(({label:e,type:r,error:o,isDirty:a,success:i,infoMessage:n,fullWidth:l,...u},p)=>{const h={...u,ref:p},[s,b]=x.useState(!1);return t.jsxs(De,{fullWidth:l,children:[e&&t.jsxs("div",{className:"label-container",children:[t.jsx(w,{as:"label",align:"left",size:"sm",color:"NEUTRAL/700",children:e}),r==="password"&&t.jsx("span",{onClick:()=>b(!s),children:s?t.jsx(je,{size:20}):t.jsx(Ue,{size:20})})]}),t.jsx(j,{error:Boolean(o),success:i||!o&&a,infoMessage:n,fullWidth:l,type:s?"text":r,...h}),o&&t.jsx(U,{shouldAppear:Boolean(o),children:t.jsx(w,{size:"xs",color:"ALERT/900",children:o})})]})});G.displayName="TextField";const B=(e,r)=>{x.useEffect(()=>{const o=a=>{if(Array.isArray(e)){for(const i of e)if(!a.target||!i.current||i.current.contains(a.target))return}else if(!a.target||!e.current||e.current.contains(a.target))return;r(a)};return document.addEventListener("mousedown",o),document.addEventListener("touchstart",o),()=>{document.removeEventListener("mousedown",o),document.removeEventListener("touchstart",o)}},[e,r])},Q=e=>t.jsx("svg",{height:e.size||16,width:e.size||16,fill:"none",viewBox:"0 0 16 16",...e,children:t.jsx("path",{d:"M12 5.333v2.534l-4 3.067-4-3.067V5.333L8 8.4l4-3.067Z",fill:"#6B6B6B"})}),Oe=e=>t.jsx("svg",{height:13,width:14,fill:"none",viewBox:"0 0 14 13",...e,children:t.jsx("path",{fillRule:"evenodd",clipRule:"evenodd",d:"M5.821 1.606a4.715 4.715 0 1 0 0 9.43 4.715 4.715 0 0 0 0-9.43ZM.125 6.32A5.696 5.696 0 1 1 10.178 9.99l2.553 2.547a.49.49 0 0 1-.693.695l-2.553-2.55A5.696 5.696 0 0 1 .125 6.32Z",fill:"#242954"})}),Ve=f.default.div`
  position: relative;
  width: ${({fullWidth:e})=>e?"100%":"auto"};
  ::selection {
    color: #fff;
    background: #4285f4;
  }
  .select-btn,
  li {
    display: flex;
    align-items: center;
    cursor: pointer;
  }
  .select-btn {
    padding: 0 20px;
    width: 100%;
    padding: 10px 12px;
    min-height: 45px;
    background: #fff;
    border: 1px solid ${()=>c.color.NEUTRAL[300]};
    border-radius: 0.5rem;
    justify-content: space-between;
  }

  .wrapper.active .select-btn i {
    transform: rotate(-180deg);
  }

  .content .options {
    margin-top: 10px;
    max-height: 250px;
    overflow-y: auto;
    padding-right: 7px;
  }

  li.selected {
    border-radius: 0.5rem;
    background-color: ${()=>c.color.QUATERNARY[200]};
  }

  .options li:hover {
    border-radius: 0.5rem;
    background-color: ${()=>c.color.QUATERNARY[100]};
  }
`,Xe=f.default.ul`
  width: ${({fullWidthOptions:e})=>e?"100%":"15rem"};
  max-height: ${({maxOptionsHeight:e})=>e?`${e}px`:"auto"};
  display: flex;
  flex-direction: column;
  background-color: #fff;
  gap: 0.5rem;
  position: absolute;
  z-index: 1;
  border-radius: 0.5rem;
  box-shadow: 0px 10px 30px rgba(0, 0, 0, 0.08);
  padding: 0;
  padding: 0.438rem 0.313rem;
  .options {
    margin: 0;
    padding: 2px 0;
    overflow-y: auto;
    li {
      padding: 0.875rem 0.5rem;
      margin: 0.5rem 0.313rem;
      &:focus {
        border-radius: 0.5rem;
        outline: 2px solid ${()=>c.color.SECONDARY[900]};
      }
    }
  }
`,R=x.forwardRef(({label:e,searchable:r=!0,autoSize:o,value:a,options:i,placeholder:n="Seleccione una opci\xF3n",fullWidth:l,fullWidthOptions:u,onChange:p,formatOptionLabel:h,formatSelectedOption:s})=>{const b=x.useRef(null),[k,v]=x.useState(i),[T,F]=x.useState(""),[y,L]=x.useState(!1);B(b,()=>{L(!1)});const S=x.useCallback(()=>{const m=i.filter(A=>A.label.toLowerCase().includes(T.toLowerCase()));v(m)},[i,a]),ne=x.useCallback(m=>{p(i.find(A=>A.label===m)),L(!1),F(""),v(i)},[S]),Vt=x.useCallback(m=>{F(m.target.value);const A=i.filter(ie=>ie.label.toLowerCase().includes(m.target.value.toLowerCase()));v(A)},[]),Xt=()=>{const m=b.current;if(m){const A=m.getBoundingClientRect();return window.innerHeight-A.bottom-20}};return t.jsxs(Ve,{fullWidth:l,ref:b,children:[e&&t.jsx(w,{as:"label",align:"left",size:"sm",color:"NEUTRAL/700",pl:"0.05rem",mb:"0.25rem",children:e}),t.jsxs("div",{className:"select-btn",onClick:()=>L(!y),tabIndex:0,onKeyUpCapture:m=>{m.key==="Enter"&&L(!y)},children:[t.jsx(w,{as:"span",color:"NEUTRAL/700",mr:"0.5rem",children:s?s(a):h?h(a):a.label}),t.jsx(Q,{size:20})]}),y&&t.jsxs(Xe,{maxOptionsHeight:o?Xt():285,fullWidthOptions:u,children:[r&&t.jsx("div",{className:"search-box",children:t.jsx(j,{leftAddon:t.jsx(Oe,{}),type:"text",placeholder:n,value:T,onChange:Vt,fullWidth:!0})}),t.jsx("div",{className:"options",children:k.map(m=>t.jsx(w,{as:"li",color:"NEUTRAL/700",className:m.value===a.value?"selected":"",tabIndex:0,onClick:()=>ne(m.label),onKeyUpCapture:A=>{A.key==="Enter"&&ne(m.label)},children:h?h(m):m.label},`${m.label}-${m.value}`))})]})]})});R.displayName="CustomSelect";const Ze=f.default.div`
  display: flex;
  flex-direction: column;
  align-items: center;
  gap: 30px;
  .profile-input__container {
    display: flex;
    position: relative;
    input[type="file"] {
      position: absolute;
      background-color: red;
      width: ${({size:e})=>e}px;
      height: ${({size:e})=>e}px;
      border-radius: 50%;
      cursor: pointer;
      opacity: 0;
      &::-webkit-file-upload-button {
        visibility: hidden;
      }
    }
    img {
      border-radius: 50%;
    }
    button {
      position: absolute;
      height: ${({size:e})=>e*.3515625}px;
      width: ${({size:e})=>e*.3515625}px;
      filter: drop-shadow(0px 2px 4px rgba(0, 0, 0, 0.25));
      bottom: 0;
      right: -5px;
    }
  }
`,Je=({imageUrl:e,size:r=128,onChange:o})=>{const a=x.useRef(null),i=n=>{var u;const l=(u=n.target.files)==null?void 0:u[0];if(l){const p=URL.createObjectURL(l);o({file:l,url:p})}};return t.jsxs(Ze,{size:r,children:[t.jsx(w,{color:"NEUTRAL/400",weight:"regular",size:"xs",children:"Agrega tu foto"}),t.jsxs("div",{className:"profile-input__container",children:[t.jsx("input",{type:"file",accept:"image/png, image/jpeg",onChange:i,ref:a}),t.jsx("img",{width:r,height:r,src:e||"/default-profile.svg",alt:"profile",referrerPolicy:"no-referrer"}),t.jsx(z,{type:"button",size:"lg",design:"flat",shape:"circular",onClick:()=>{var n;return(n=a.current)==null?void 0:n.click()},children:"+"})]})]})},qe=d.keyframes`
  0%,
  100% {
    animation-timing-function: cubic-bezier(0.2 0 0.8 0.8);
  }
  50% {
    animation-timing-function: cubic-bezier(0.2 0.2 0.8 1);
  }
  0% {
    transform: rotate(0deg);
  }
  50% {
    transform: rotate(180deg);
  }
  100% {
    transform: rotate(360deg);
  }
`,Ge=d.keyframes`
  0%,
  100% {
    animation-timing-function: cubic-bezier(0.2 0 0.8 0.8);
  }
  50% {
    animation-timing-function: cubic-bezier(0.2 0.2 0.8 1);
  }
  0% {
    transform: translate(-30px, -30px) scale(0);
  }
  50% {
    transform: translate(-30px, -30px) scale(1);
  }
  100% {
    transform: translate(-30px, -30px) scale(0);
  }
`,Be=f.default.div`
  width: 200px;
  height: 200px;
  display: inline-block;
  overflow: hidden;
  background-color: transparent;

  .idbi-loading {
    width: 100%;
    height: 100%;
    position: relative;
    transform: translateZ(0) scale(1);
    backface-visibility: hidden;
    transform-origin: 0 0; /* see note above */
  }

  .idbi-loading div {
    box-sizing: content-box;
  }

  .idbi-loading > div {
    transform: translate(0px, -15px);
  }
  .idbi-loading > div > div {
    animation: ${qe} 1s linear infinite;
    transform-origin: 100px 100px;
  }
  .idbi-loading > div > div > div {
    position: absolute;
    transform: translate(100px, 82px);
  }
  .idbi-loading > div > div > div > div {
    width: 60px;
    height: 60px;
    border-radius: 50%;
    background-color: ${()=>c.color.PRIMARY[900]};
    animation: ${Ge} 1s linear infinite;
  }
  .idbi-loading > div > div:last-child {
    animation-delay: -0.5s;
  }
  .idbi-loading > div > div:last-child > div > div {
    animation-delay: -0.5s;
    background-color: ${()=>c.color.TERTIARY[900]};
  }
`,Qe=()=>t.jsx(Be,{children:t.jsx("div",{className:"idbi-loading",children:t.jsxs("div",{children:[t.jsx("div",{children:t.jsx("div",{children:t.jsx("div",{})})}),t.jsx("div",{children:t.jsx("div",{children:t.jsx("div",{})})})]})})}),Re=f.default.section`
  width: 100%;
  border-radius: 0.625rem;
  header {
    border-bottom: 1px solid #e5e5e5;
  }
  ul {
    margin: 0;
    padding: 0;
    & li {
      list-style: none;
      margin: 0;
      padding: 0;
    }

    & li > *:first-child {
      border-radius: 0;
    }

    & li:first-child > *:first-child {
      border-top-left-radius: 0.625rem;
      border-top-right-radius: 0.625rem;
      border-bottom-left-radius: 0;
      border-bottom-right-radius: 0;
    }

    & li:last-child > *:first-child {
      border-bottom-left-radius: 0.625rem;
      border-bottom-right-radius: 0.625rem;
      border-top-left-radius: 0;
      border-top-right-radius: 0;
    }

    ${({withSeparator:e})=>e&&d.css`
        & li:not(:last-child) {
          border-bottom: 1px solid #e5e5e5;
        }
      `}
  }
  ${({design:e})=>{switch(e){case"primary":return d.css`
          background-color: #fff;
          box-shadow: 0px 10px 30px rgba(0, 0, 0, 0.08);
          -webkit-box-shadow: 0px 10px 30px rgba(0, 0, 0, 0.08);
          -moz-box-shadow: 0px 10px 30px rgba(0, 0, 0, 0.08);
        `;case"secondary":return d.css`
          background-color: ${c.color.QUATERNARY[300]};
        `;case"flat":return d.css`
          background-color: transparent;
        `;default:return c.color.NEUTRAL[100]}}};
`,Ke=({design:e="primary",header:r,options:o,optionsFormatter:a,content:i,withSeparator:n})=>t.jsxs(Re,{design:e,withSeparator:n,children:[r&&t.jsx("header",{children:r}),o&&a&&t.jsx("ul",{children:o.map(l=>t.jsx("li",{children:a(l)},l.value))}),i&&i]}),$e=e=>t.jsx("svg",{width:e.size||24,height:e.size||25,fill:"none",viewBox:"0 0 24 25",...e,children:e.ischecked?t.jsxs(t.Fragment,{children:[t.jsx("path",{fillRule:"evenodd",clipRule:"evenodd",d:"M22.53 3.97a.75.75 0 0 1 0 1.06l-10 10a.75.75 0 0 1-1.06 0l-3-3a.75.75 0 1 1 1.06-1.06L12 13.44l9.47-9.47a.75.75 0 0 1 1.06 0Z",fill:"#4318FF"}),t.jsx("path",{fillRule:"evenodd",clipRule:"evenodd",d:"M5 4.25A1.25 1.25 0 0 0 3.75 5.5v14A1.25 1.25 0 0 0 5 20.75h14a1.25 1.25 0 0 0 1.25-1.25v-7a.75.75 0 0 1 1.5 0v7A2.75 2.75 0 0 1 19 22.25H5a2.75 2.75 0 0 1-2.75-2.75v-14A2.75 2.75 0 0 1 5 2.75h11a.75.75 0 0 1 0 1.5H5Z",fill:"#4318FF"})]}):t.jsx("path",{fillRule:"evenodd",clipRule:"evenodd",d:"M5 4.25c-.69 0-1.25.56-1.25 1.25v14c0 .69.56 1.25 1.25 1.25h14c.69 0 1.25-.56 1.25-1.25v-14c0-.69-.56-1.25-1.25-1.25H5ZM2.25 5.5A2.75 2.75 0 0 1 5 2.75h14a2.75 2.75 0 0 1 2.75 2.75v14A2.75 2.75 0 0 1 19 22.25H5a2.75 2.75 0 0 1-2.75-2.75v-14Z",fill:"#4318FF"})}),_e=f.default(E.Root)`
  width: 100%;
  background-color: #fff;
  box-shadow: ${()=>c.effect.darkShadow[4]};
  border-radius: 0.5rem;
`,et=f.default(E.Trigger)`
  display: flex;
  align-items: center;
  justify-content: space-between;
  width: 100%;
  padding: 1rem;
  cursor: pointer;
  background-color: transparent;
  border: none;
`,tt=d.keyframes`
  from {
    height: 0;
  }
  to {
    height: var(--radix-collapsible-content-height);
  }
`,rt=d.keyframes`
  from {
    height: var(--radix-collapsible-content-height);
  }
  to {
    height: 0;
  }
`,ot=f.default(E.Content)`
  overflow: hidden;
  &[data-state="open"] {
    animation: ${tt} 300ms ease-out;
  }
  &[data-state="closed"] {
    animation: ${rt} 300ms ease-out;
  }
`,nt=({open:e,setOpen:r,label:o,content:a})=>t.jsxs(_e,{open:e,onOpenChange:r,children:[t.jsxs(et,{children:[t.jsx(w,{children:o}),t.jsx($e,{ischecked:e||!1})]}),t.jsx(ot,{children:a})]}),I=e=>t.jsx("svg",{width:e.size||8,height:e.size||12,fill:"none",viewBox:"0 0 8 12",onClick:e.onClick,...e,children:t.jsx("path",{d:"m3.069 6.003 4.73-4.49a.634.634 0 0 0 0-.928L7.384.192A.705.705 0 0 0 6.895 0a.705.705 0 0 0-.489.192L.774 5.537a.634.634 0 0 0-.202.465c0 .177.071.342.202.466l5.627 5.34c.13.124.304.192.49.192a.706.706 0 0 0 .488-.192l.415-.393a.634.634 0 0 0 0-.928L3.069 6.003Z",fill:"#151E5A"})}),it=f.default.header`
  display: flex;
  justify-content: space-between;
  align-items: center;
  width: 100%;
  position: sticky;
  top: 0;
  background-color: #fff;
  z-index: 1;
  padding-left: 1.5rem;
  padding-right: 1.5rem;
  box-shadow: ${()=>c.effect.darkShadow[6]};

  button {
    display: flex;
    align-items: center;
    background-color: transparent;
    border: none;
    padding: 0;
    margin: 0;
    cursor: pointer;
    gap: 0.5rem;
    height: 100%;
    padding-top: 0.75rem;
    padding-bottom: 0.75rem;
  }

  img {
    border-radius: 50%;
  }
`,at=({onBack:e,backLabel:r,profileImageUrl:o,rightContent:a})=>t.jsxs(it,{children:[t.jsxs("button",{type:"button",onClick:e,children:[t.jsx(I,{}),t.jsx(w,{color:"NEUTRAL/900",children:r})]}),a||t.jsx("img",{width:28,height:28,src:o||"/default-profile.svg",alt:"profile",referrerPolicy:"no-referrer"})]}),dt=d.keyframes`
  100% {
    transform: rotate(360deg);
  }
`,lt=f.default.div`
  display: block;
  margin: ${e=>e.margin||"25% auto"};
  width: ${e=>e.width||"100%"};
  height: ${e=>e.height||"100%"};
  border: ${e=>e.borderWidth||"0.25em"} solid rgba(0, 0, 0, 0.1);
  border-top-color: #03a7e5;
  border-radius: 50%;
  animation: ${dt} 1s infinite linear;
`,M=e=>t.jsx(lt,{width:e.width,height:e.height,borderWidth:e.borderWidth,margin:e.margin}),K=f.default.ul`
  color: ${()=>c.color.OTHER.color1};
  background-color: white;
  box-shadow: 0px 10px 30px rgba(0, 0, 0, 0.08);
  border: 1px solid #59a5fc;
  border-radius: 10px;
  width: 300px;
  margin: 5px 0;
  padding: 0;
  z-index: 99;
  font: 400 14px Poppins;
  list-style-type: none;

  & > li:last-child {
    border-radius: 0 0 10px 10px;
  }

  & > li:first-child {
    border-radius: 10px 10px 0 0;
  }
`,H=f.default.li`
  background-color: ${({active:e})=>e?"#ebedff":"white"};
  min-height: 50px;
  display: flex;
  align-items: center;
  justify-content: space-between;
  padding: 5px 10px 5px 15px;
  gap: 7px;
  opacity: ${({disabled:e})=>e?.6:1};
  position: relative;
  @media (hover: hover) {
    cursor: ${({disabled:e})=>e?"not-allowed":"pointer"};
  }

  &:hover {
    background-color: #ebedff;
  }

  & > svg,
  & > img {
    flex-shrink: 0;
  }
  & > img {
    width: 18px;
    height: 18px;
    object-fit: contain;
  }
  & > svg {
    font-size: 20px;
  }

  ${({alert:e})=>{if(e)return d.css`
        background-color: ${c.color.ALERT[100]};
        color: ${c.color.ALERT[900]};
        &:hover {
          background-color: ${c.color.ALERT[200]};
        }
      `}}
`,ct=f.default.div`
  position: absolute;
  top: 0;
  right: calc(100% + 8px);
`,st=({text:e,options:r})=>{const[o,a]=x.useState(!1),i=x.useRef(null),n=l=>{if(i.current&&clearTimeout(i.current),l){a(!0);return}i.current=setTimeout(()=>{o!==l&&a(l)},50)};return t.jsxs(H,{onMouseEnter:()=>n(!0),onMouseLeave:()=>n(!1),active:o,onClick:l=>l.stopPropagation(),children:[e," ",t.jsx("div",{style:{transform:"rotateY(180deg)"},children:t.jsx(I,{})}),o&&t.jsx(ct,{children:t.jsx(K,{children:r.map((l,u)=>t.jsxs(H,{alert:l.alert,onClick:p=>l.disabled||l.loading?p.stopPropagation():l.onClick(),disabled:l.disabled||l.loading,loading:l.loading,children:[l.text," ",l.loading?t.jsx("div",{style:{width:"24px"},children:t.jsx(M,{width:"20px",height:"20px"})}):l.icon]},`suboption-${u}`))})})]})},ft=({options:e})=>t.jsx(K,{children:e.map((r,o)=>"options"in r?t.jsx(st,{...r},`option-${o}`):t.jsxs(H,{disabled:r.disabled||r.loading,alert:r.alert,onClick:r.disabled||r.loading?a=>a.stopPropagation():r.onClick,loading:r.loading,children:[r.text," ",r.loading?t.jsx("div",{style:{width:"24px"},children:t.jsx(M,{width:"20px",height:"20px"})}):r.icon]},`option-${o}`))}),ht=f.default.div`
  cursor: pointer;
  text-align: right;
  * {
    margin-left: auto;
    margin-right: 0;
  }
`,ut=({options:e,activator:r})=>{const{x:o,y:a,reference:i,floating:n,strategy:l,update:u,refs:p}=D.useFloating({placement:"bottom-end"}),[h,s]=x.useState(!1);return x.useEffect(()=>{if(!(!p.reference.current||!p.floating.current))return D.autoUpdate(p.reference.current,p.floating.current,u)},[h,u]),B(p.reference,()=>{s(!1)}),t.jsxs("div",{ref:i,children:[t.jsx(ht,{onClick:()=>s(!h),children:r||t.jsx(Q,{size:25})}),h&&t.jsx("div",{ref:n,style:{position:l,top:a!=null?a:0,left:o!=null?o:0},onClick:()=>s(!h),children:t.jsx(ft,{options:e})})]})},pt=e=>t.jsx("svg",{width:9,height:9,fill:"none",xmlns:"http://www.w3.org/2000/svg",...e,children:t.jsx("path",{d:"M8.53 1.53A.75.75 0 0 0 7.47.47l1.06 1.06ZM.47 7.47a.75.75 0 0 0 1.06 1.06L.47 7.47Zm7 1.06a.75.75 0 0 0 1.06-1.06L7.47 8.53ZM1.53.47A.75.75 0 0 0 .47 1.53L1.53.47Zm5.94 0-7 7 1.06 1.06 7-7L7.47.47Zm1.06 7-7-7L.47 1.53l7 7 1.06-1.06Z",fill:"#404040"})}),gt=f.default.div`
  background-color: ${e=>`rgba(0, 0, 0, ${e.opacity?e.opacity:"0.5"})`};
  width: 100vw;
  height: 100vh;
  position: fixed;
  top: 0;
  left: 0;
  z-index: ${e=>e.zIndex?e.zIndex:"auto"};
  cursor: auto;
  animation: fadeIn 0.3s ease-in-out;
  @keyframes fadeIn {
    from {
      opacity: 0;
    }
    to {
      opacity: 1;
    }
  }
`,xt=({onClose:e=()=>{},backdropOpacity:r,zIndex:o})=>(x.useEffect(()=>(document.body.style.overflow="hidden",()=>{document.body.style.overflow="visible"}),[]),t.jsx(gt,{onClick:a=>{a.stopPropagation(),e()},opacity:r,zIndex:o})),$=d.keyframes`
  from {
    transform: translateY(100px);
    opacity: 0;
  }
  to {
    transform: translateY(0);
    opacity: 1;
  }
`,_=d.keyframes`
  from {
    transform: translateY(-100px);
    opacity: 0;
  }
  to {
    transform: translateY(0);
    opacity: 1;
  }
`,ee=d.keyframes`
  from {
    transform: translateX(-100px);
    opacity: 0;
  }
  to {
    transform: translateX(0);
    opacity: 1;
  }
`,te=d.keyframes`
  from {
    transform: translateX(100px);
    opacity: 0;
  }
  to {
    transform: translateX(0);
    opacity: 1;
  }
`,bt=f.default.div`
  position: fixed;
  max-height: 100vh;
  max-width: 100vw;
  overflow: auto;
  animation: fadeIn 0.3s ease-in-out;
  z-index: ${e=>e.zIndex};
  top: 50%;
  left: 50%;
  transform: translate(-50%, -50%);

  @keyframes fadeIn {
    from {
      opacity: 0;
    }
    to {
      opacity: 1;
    }
  }

  ${e=>{if(e.contentLocation==="bottom")return d.css`
        transform: translate(0, 0);
        right: unset;
        top: unset;
        left: 0;
        bottom: 0;
        animation: ${$} 0.3s ease-out;
      `;if(e.contentLocation==="top")return d.css`
        transform: translate(0, 0);
        right: unset;
        bottom: unset;
        left: 0;
        top: 0;
        animation: ${_} 0.3s ease-out;
      `;if(e.contentLocation==="left")return d.css`
        transform: translate(0, 0);
        top: unset;
        right: unset;
        left: 0;
        bottom: 0;
        animation: ${ee} 0.3s ease-out;
      `;if(e.contentLocation==="right")return d.css`
        transform: translate(0, 0);
        top: unset;
        left: unset;
        right: 0;
        bottom: 0;
        animation: ${te} 0.3s ease-out;
      `}}

  ${e=>{if(e.contentLocationInDesktop==="center")return d.css`
        @media screen and (min-width: 1024px) {
          transform: translate(-50%, -50%);
          bottom: unset;
          right: unset;
          top: 50%;
          left: 50%;
          animation: fadeIn 0.3s ease-out;
        }
      `;if(e.contentLocationInDesktop==="bottom")return d.css`
        @media screen and (min-width: 1024px) {
          transform: translate(0, 0);
          top: unset;
          right: unset;
          left: 0;
          bottom: 0;
          animation: ${$} 0.3s ease-out;
        }
      `;if(e.contentLocationInDesktop==="top")return d.css`
        @media screen and (min-width: 1024px) {
          transform: translate(0, 0);
          top: unset;
          right: unset;
          left: 0;
          top: 0;
          animation: ${_} 0.3s ease-out;
        }
      `;if(e.contentLocationInDesktop==="left")return d.css`
        @media screen and (min-width: 1024px) {
          transform: translate(0, 0);
          top: unset;
          right: unset;
          left: 0;
          bottom: 0;
          animation: ${ee} 0.3s ease-out;
        }
      `;if(e.contentLocationInDesktop==="right")return d.css`
        @media screen and (min-width: 1024px) {
          transform: translate(0, 0);
          top: unset;
          left: unset;
          right: 0;
          bottom: 0;
          animation: ${te} 0.3s ease-out;
        }
      `}}
`,mt=({children:e,onClose:r,backdropOpacity:o,zIndex:a,contentLocation:i="center",contentLocationInDesktop:n="center"})=>t.jsxs(t.Fragment,{children:[t.jsx(xt,{onClose:r,backdropOpacity:o,zIndex:a||4}),t.jsx(bt,{zIndex:a||4,contentLocation:i,contentLocationInDesktop:n,onClick:l=>l.stopPropagation(),children:e})]}),vt=f.default.div`
  background-color: ${e=>e.background||"#fff"};
  border-radius: ${e=>e.borderRadius?e.borderRadius:"15px"};
  max-height: ${e=>e.maxHeight?e.maxHeight:"90vh"};
  height: ${e=>e.height?e.height:"auto"};
  max-width: ${e=>e.maxWidth?e.maxWidth:"90vw"};
  width: ${e=>e.width?e.width:"90vw"};
  padding: ${e=>e.padding?e.padding:"1.5rem"};
  overflow: ${e=>e.overflow||"hidden"};
  cursor: auto;
  box-shadow: 0px 0px 24px 4px rgba(0, 12, 51, 0.05);
  ${e=>{if(e.contentLocation&&e.contentLocation==="bottom")return d.css`
        border-bottom-left-radius: 0;
        border-bottom-right-radius: 0;
        max-width: unset;
        width: 100vw;
      `;if(e.contentLocation&&e.contentLocation==="top")return d.css`
        border-top-left-radius: 0;
        border-top-right-radius: 0;
        max-width: unset;
        width: 100vw;
      `;if(e.contentLocation&&e.contentLocation==="left")return d.css`
        border-top-left-radius: 0;
        border-bottom-left-radius: 0;
        height: 100vh;
        max-height: ${()=>e.maxHeight?e.maxHeight:"100vh"};
        max-width: ${()=>e.maxWidth?e.maxWidth:"550px"};
      `;if(e.contentLocation&&e.contentLocation==="right")return d.css`
        border-top-right-radius: 0;
        border-bottom-right-radius: 0;
        height: 100vh;
        max-height: ${()=>e.maxHeight?e.maxHeight:"100vh"};
        max-width: ${()=>e.maxWidth?e.maxWidth:"550px"};
      `}}

  ${e=>{if(e.contentLocationInDesktop==="center")return d.css`
        @media screen and (min-width: 1024px) {
          border-radius: ${()=>e.borderRadiusInDesktop?e.borderRadiusInDesktop:"15px"};
          height: auto;
          max-width: ${()=>e.maxWidth?e.maxWidth:"90vw"};
          max-height: ${()=>e.maxHeight?e.maxHeight:"90vh"};
          padding: ${()=>e.padding?e.padding:"1.5rem"};
        }
      `;if(e.contentLocationInDesktop==="bottom")return d.css`
        @media screen and (min-width: 1024px) {
          border-radius: ${()=>e.borderRadiusInDesktop?e.borderRadiusInDesktop:"15px"};
          border-bottom-left-radius: 0;
          border-bottom-right-radius: 0;
          max-width: unset;
          height: unset;
          width: ${()=>e.width?e.width:"100vw"};
        }
      `;if(e.contentLocationInDesktop==="top")return d.css`
        @media screen and (min-width: 1024px) {
          border-radius: ${()=>e.borderRadiusInDesktop?e.borderRadiusInDesktop:"15px"};
          border-top-left-radius: 0;
          border-top-right-radius: 0;
          max-width: unset;
          height: unset;
          width: ${()=>e.width?e.width:"100vw"};
        }
      `;if(e.contentLocationInDesktop==="left")return d.css`
        @media screen and (min-width: 1024px) {
          border-radius: ${()=>e.borderRadiusInDesktop?e.borderRadiusInDesktop:"15px"};
          border-top-left-radius: 0;
          border-bottom-left-radius: 0;
          height: 100vh;
          max-height: ${()=>e.maxHeight?e.maxHeight:"100vh"};
          max-width: ${()=>e.maxWidth?e.maxWidth:"550px"};
        }
      `;if(e.contentLocationInDesktop==="right")return d.css`
        @media screen and (min-width: 1024px) {
          border-radius: ${()=>e.borderRadiusInDesktop?e.borderRadiusInDesktop:"15px"};
          border-top-right-radius: 0;
          border-bottom-right-radius: 0;
          height: 100vh;
          max-height: ${()=>e.maxHeight?e.maxHeight:"100vh"};
          max-width: ${()=>e.maxWidth?e.maxWidth:"550px"};
        }
      `}}
`,wt=f.default.div`
  background-color: white;
  box-shadow: 0 2px 7px #e5e5e5;
  border-radius: 50%;
  width: 28px;
  height: 28px;
  display: ${({closeIcon:e})=>e?"flex":"none"};
  justify-content: center;
  align-items: center;
  position: absolute;
  top: 20px;
  right: 20px;
  font-size: 11px;
  cursor: pointer;
  @media screen and (min-width: 1024px) {
    display: ${({closeIconInDesktop:e})=>e?"flex":"none"};
  }
`,re=({children:e,onClose:r,backdropOpacity:o,padding:a,borderRadius:i,borderRadiusInDesktop:n,maxHeight:l,height:u,maxWidth:p,width:h,zIndex:s,overflow:b,contentLocation:k="center",contentLocationInDesktop:v="center",closeIcon:T,closeIconInDesktop:F,background:y="#fff"})=>t.jsx(mt,{onClose:r,backdropOpacity:o,zIndex:s,contentLocation:k,contentLocationInDesktop:v,children:t.jsxs(vt,{padding:a,borderRadius:i,borderRadiusInDesktop:n,maxHeight:l,height:u,maxWidth:p,width:h,background:y,overflow:b,contentLocation:k,contentLocationInDesktop:v,children:[(T||F)&&t.jsx(wt,{onClick:r,closeIconInDesktop:F,closeIcon:T,children:t.jsx(pt,{})}),e]})}),kt=f.default.div`
  @media (max-width: 600px) {
    padding-bottom: 15px;
  }
`,Ct=f.default.div`
  display: flex;
  align-items: center;
  gap: 10px;
`,At=f.default.div`
  padding: 0 20px;
  & > :nth-child(2) {
    margin-top: 5px;
  }

  svg,
  img {
    cursor: pointer;
  }
`,Nt=f.default.ul`
  color: ${()=>c.color.NEUTRAL[700]};
  padding: 0;
  margin: 10px 0;
  list-style-type: none;
  font-size: 14px;
`,Y=f.default.li`
  background-color: ${({alert:e})=>e?c.color.ALERT[100]:"white"};
  color: ${({alert:e})=>e?c.color.ALERT[900]:"inherit"};
  min-height: 50px;
  padding: 0 20px;
  display: flex;
  justify-content: ${({optionsInCenter:e})=>e?"center":"space-between"};
  align-items: center;
  gap: ${({optionsInCenter:e})=>e?"10px":"5px"};
  opacity: ${({disabled:e})=>e?.6:1};
  cursor: ${({disabled:e})=>e?"not-allowed":"pointer"};;

  & > svg,
  & > img {
    flex-shrink: 0;
  }
  & > img {
    width: 18px;
    height: 18px;
    object-fit: contain;
  }
  & > svg {
    font-size: 20px;
  }
`,oe=t.jsx("div",{style:{display:"inline-block"},children:t.jsx(M,{width:"20px",height:"20px"})}),Ft=({options:e,onClose:r,contentLocation:o,contentLocationInDesktop:a,defaultDescription:i="",defaultTitle:n="",optionsInCenter:l=!1})=>{const[u,p]=x.useState(null),h=x.useMemo(()=>{if(u===null)return null;const s=e[u];if(s&&"options"in s)return s},[u,e]);return t.jsx(re,{maxWidth:"500px",padding:"20px 0",onClose:r,contentLocation:o||"bottom",contentLocationInDesktop:a||"center",closeIcon:!0,children:t.jsxs(kt,{children:[t.jsxs(At,{children:[t.jsxs(Ct,{children:[h&&t.jsx(I,{onClick:()=>p(null)}),t.jsx(w,{as:"h2",size:"md",children:(h==null?void 0:h.text)||n})]}),i.length>0&&t.jsx(w,{color:"NEUTRAL/400",size:"sm",weight:"regular",children:(h==null?void 0:h.description)||i})]}),t.jsxs(Nt,{children:[!h&&e.map((s,b)=>"onClick"in s?t.jsxs(Y,{alert:s.alert,disabled:s.disabled||s.loading,onClick:()=>{!s.loading&&!s.disabled&&(s.onClick(),r())},optionsInCenter:l,children:[s.text," ",s.loading?oe:s.icon]},`option-${b}`):t.jsxs(Y,{onClick:()=>{p(b)},optionsInCenter:l,children:[s.text," ",t.jsx("div",{style:{transform:"rotateY(180deg)"},children:t.jsx(I,{})})]},`option-${b}`)),h==null?void 0:h.options.map((s,b)=>t.jsxs(Y,{alert:s.alert,disabled:s.disabled||s.loading,onClick:()=>{!s.loading&&!s.disabled&&s.onClick()},optionsInCenter:l,children:[s.text," ",s.loading?oe:s.icon]},`suboption-${b}`))]})]})})},yt=f.default.div`
  padding: 20px;
  background: linear-gradient(#feedec 25%, #fff 25%);
  box-shadow: 0px 10px 30px rgba(0, 0, 0, 0.08);
  border-radius: 20px;
  text-align: center;
  font-weight: 500;
  font-size: 1rem;
  line-height: 1.5rem;
  color: #383838;
`,zt=f.default.p`
  font-weight: 400;
  font-size: 0.875rem;
  line-height: 21px;
  color: #808080;
  margin: 15px 0;
`,Tt=f.default.div`
  margin-top: 30px;
  padding-top: 20px;
  display: flex;
  justify-content: space-around;
  gap: 10px;
  border-top: 1px solid #dfdfdf;
`,Lt=f.default.div`
  width: 3px;
  background: #dfdfdf;
`,St=f.default.div`
  padding: 0 20px;
`,It="data:image/webp;base64,",Wt=({height:e})=>t.jsx("img",{src:It,height:e||45,alt:"trash"}),Et=({children:e,icon:r=t.jsx(Wt,{height:150}),note:o="Esta acci\xF3n no se puede revertir.",btnLeft:a={onClick:()=>{},text:"Cancelar"},btnRight:i={onClick:()=>{},text:"Eliminar"}})=>t.jsxs(yt,{children:[r,t.jsx(St,{children:e}),t.jsx(zt,{children:o}),t.jsxs(Tt,{children:[t.jsx(z,{design:"flat",fullWidth:!0,onClick:()=>a.onClick(),children:a.text}),t.jsx(Lt,{}),t.jsx(z,{design:"flat",fullWidth:!0,onClick:()=>i.onClick(),color:"ALERT/900",children:i.text})]})]}),Pt=f.default.div`
  box-shadow: 0px 10px 30px rgba(0, 0, 0, 0.08);
  border-radius: 20px;
  font-weight: 500;
  font-size: 1rem;
  line-height: 1.5rem;
  color: #383838;
  background: transparent;
  @media screen and (min-width: 1024px) {
    text-align: center;
    background: #fff;
  }

  button {
    font-family: "Poppins";
    font-style: normal;
  }
`,Ut=f.default.div`
  border-radius: 15px 15px 0 0;
  background: #f3f5ff;
  padding: 20px;
  @media screen and (min-width: 1024px) {
    padding: 50px 58px;
  }
`,jt=f.default.p`
  font-weight: 400;
  font-size: 0.875rem;
  line-height: 21px;
  color: #808080;
  margin: 5px 0 0;
`,Mt=f.default.div`
  padding: 20px;
  @media screen and (min-width: 1024px) {
    display: flex;
    gap: 10px;
    flex-direction: row;
    justify-content: space-around;
    padding: 27px 29px;
  }
`,Ht=f.default.div`
  font-family: "Poppins";
  font-style: normal;
  font-weight: 700;
  font-size: 1.25rem;
  line-height: 1.6875rem;
  letter-spacing: -0.04em;
  color: #151e5a;
`,Yt=f.default.div`
  margin: 0 15px 15px;
  background: #fff;
  border-radius: 15px;
  @media screen and (min-width: 1024px) {
    margin: 0;
  }
`;function Dt(e="(min-width: 768px)"){const[r,o]=x.useState({matches:window.innerWidth>(()=>{const i=e.match(/(\d+)/);return i?i[0]:768})()});return x.useEffect(()=>{let i=window.matchMedia(e);return i.addEventListener("change",o),()=>i.removeEventListener("change",o)},[]),{status:Boolean(r&&r.matches)}}const Ot=({children:e,note:r="Esta acci\xF3n no se puede revertir.",btnLeft:o={onClick:()=>{},text:"Cancelar"},btnRight:a={onClick:()=>{},text:"Eliminar"}})=>{const{status:i}=Dt("(min-width: 1023px)");return t.jsxs(Pt,{children:[t.jsxs(Yt,{children:[t.jsxs(Ut,{children:[t.jsx(Ht,{children:e}),t.jsx(jt,{children:r})]}),t.jsxs(Mt,{children:[i&&t.jsx(z,{design:"flat",fullWidth:!0,onClick:()=>o.onClick(),color:"PRIMARY/900 ",style:{borderRadius:"32px",padding:"20px 0"},children:o.text}),t.jsx(z,{design:"solid",fullWidth:!0,onClick:()=>a.onClick(),style:{borderRadius:"32px",padding:"20px 60px"},children:a.text})]})]}),!i&&t.jsx(z,{design:"flat",fullWidth:!0,onClick:()=>o.onClick(),color:"PRIMARY/900",style:{margin:"0 15px 15px",width:"calc(100% - 30px)",padding:"20px",borderRadius:"15px"},children:o.text})]})};g.Button=z,g.Card=Ke,g.CardModal=re,g.CodeInput=Le,g.Collapsible=nt,g.ConfirmationCard=Et,g.ConfirmationCardV2=Ot,g.CustomSelect=R,g.DropdownMenu=ut,g.Heading=Fe,g.Loader=Qe,g.MultipleOptionsModal=Ft,g.ProfileImage=Je,g.SelectField=We,g.StickyHeader=at,g.Text=w,g.TextField=G,Object.defineProperties(g,{__esModule:{value:!0},[Symbol.toStringTag]:{value:"Module"}})});
